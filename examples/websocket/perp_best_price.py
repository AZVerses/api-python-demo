...
if __name__ == '__main__':
    ...
    my_client.best_price(symbol=symbol, action=PerpWebsocketStreamClient.ACTION_SUBSCRIBE)
    # keep heartbeat
    threading.Thread(target=my_client.heartbeat, daemon=False).start()
    time.sleep(5)
    # # Unsubscribe
    my_client.best_price(symbol=symbol, action=PerpWebsocketStreamClient.ACTION_UNSUBSCRIBE)
    time.sleep(5)
